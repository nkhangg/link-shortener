...
export default function CreateShort({ onSubmitted }: { onSubmitted: () => void }) {
    ...
    const mutation = useMutation({
        ...
        mutationFn: async ({ url }: { url: string }) => {
            ...
            const { data, error } = await supabase
                .from('links')
                .insert([{ original_url: url } as ShortenedUrl])
    ...
    return (
        <Card className="mb-8">
            <CardHeader className="text-center">
                ...
                <CardDescription className="text-lg text-pretty">Tạo link ngắn gọn, dễ chia sẻ từ URL dài của bạn</CardDescription>
            </CardHeader>

            <CardContent className="space-y-4">
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(handleSubmit)} className="flex gap-2" noValidate>
                        <FormField
                        ...
                        <Button type="submit" className="px-8" disabled={mutation.isPending}>
                            {mutation.isPending ? <Loader color="black" /> : 'Rút gọn'}
                        </Button>
                    </form>
                </Form>
            </CardContent>
