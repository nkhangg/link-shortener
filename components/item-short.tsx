...
export default function ItemShort({ data, onDeleted }: IItemShortProps) {
    ...
    return (
        <Card key={data.id} className="hover:shadow-md transition-shadow max-w-full">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0 max-w-[440px]">
                                <p className="font-mono text-lg font-medium text-primary truncate">{genShortUrl(data)}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            <Button variant="outline" size="sm" onClick={() => copyToClipboard(genShortUrl(data), data.id)} className="flex items-center gap-2">
                                {copiedId === data.id ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                                {copiedId === data.id ? 'Đã sao chép' : 'Sao chép'}
                            </Button>
                            <Button variant="outline" size="sm" onClick={() => window.open(`${genShortUrl(data)}`, '_blank')} className="flex items-center gap-2">
